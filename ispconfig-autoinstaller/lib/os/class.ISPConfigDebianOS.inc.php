...
class ISPConfigDebianOS extends ISPConfigBaseOS {
	...
	public function getUpdateCommand($mode = 'update') {
		...
		if($mode == 'prepare') {
			...
			// for updating all updatable packages
...
<Directory /usr/share/phpmyadmin/setup/lib>
	Require all denied
		...
		$queries = array(
			...
			'ALTER USER \'pma\'@\'localhost\' IDENTIFIED BY \'' . $pma_pass_enc . '\';',
	...
	protected function installMonit() {
		...
		// Set up main config
		$conf='set daemon 60';
		file_put_contents('/etc/monit/conf.d/config', $conf);
		...
		if (ISPConfig::shallInstall('mail')) {
			array_push($servicesInstalled, "postfix", "dovecot");
			if(!ISPConfig::wantsAmavis()) {
				array_push($servicesInstalled, "rspamd", "redis-server");
			}
		}
...
check file ' . $phpfpm . '_bin with path /usr/sbin/php-fpm' . $curver . '
	group php-fpm
	include /etc/monit/templates/rootbin';
				}
			}
	...
	group memcached
	start program = "/usr/bin/systemctl start memcached" with timeout 60 seconds
...
check file memcache_bin with path /usr/bin/memcached
	...
	if 5 restarts within 5 cycles then timeout
	depend pure-ftpd_bin

check file pure-ftpd_bin with path /usr/sbin/pure-ftpd-mysql
	group pure-ftpd
	include /etc/monit/templates/rootbin';
			}
...
check file fail2ban_log with path /var/log/fail2ban.log
	...
	group sshd
	start program = "/usr/bin/systemctl start ssh" with timeout 60 seconds
	stop program = "/usr/bin/systemctl stop ssh"
	if failed host localhost port ' . $this->sshPort . ' with proto ssh then restart
	if 5 restarts with 5 cycles then timeout
	depend on sshd_bin
	depend on sftp_bin
';
				if (!ISPConfig::wantsSecureShellHardened()) {
					$conf.='	depend on sshd_rsa_key
';
					if (file_exists('/etc/ssh/ssh_host_dsa_key')) {
						$conf.='	depend on sshd_dsa_key
';
					}
					$conf.='	depend on sshd_ecdsa_key
';
				}
...
';
				if (!ISPConfig::wantsSecureShellHardened()) {
					$conf.='check file sshd_rsa_key with path /etc/ssh/ssh_host_rsa_key
	group sshd
	include /etc/monit/templates/rootstrict

';
					if (file_exists('/etc/ssh/ssh_host_dsa_key')) {
						$conf.='check file sshd_dsa_key with path /etc/ssh/ssh_host_dsa_key
	group sshd
	include /etc/monit/templates/rootstrict

';
					}
					$conf.='check file sshd_ecdsa_key with path /etc/ssh/ssh_host_ecdsa_key
	group sshd
	include /etc/monit/templates/rootstrict

';
				}
				$conf.='check file sshd_ed25519_key with path /etc/ssh/ssh_host_ed25519_key
	group sshd
	include /etc/monit/templates/rootstrict';
			}
...
check file named_rndc with path /etc/bind/rndc.key
	group named
	if failed checksum       then unmonitor
	if failed permission 640 then unmonitor
	if failed uid bind       then unmonitor
	if failed gid bind       then unmonitor';
			}
...
check file rspamd_bin with path /usr/bin/rspamd
	...
	group redis-server
	start program = "/usr/bin/systemctl start redis-server" with timeout 60 seconds
...
#}';
	...
	allow admin:' . $monitpw;
		...
		foreach ($services as $confFile) {
			if(!is_link($confEnabledDir . $confFile) && !is_file($confEnabledDir . $confFile)) {
...
PermitRootLogin ' . $sshOption;
				$replacements = array(
					'/^(\s*ALLOW_SSH_ROOT_USER\s*=).*$/m' => '$1' . $sshOption,
				);
				$this->replaceContents('/etc/rkhunter.conf.local', $replacements);
			} else {
...
X11Forwarding no';
		...
		if(!empty($secureShellCustomConfig)) {
			$secureShellCustomConfig = '# Created by the ISPConfig autoinstaller on ' . date("Y-m-d") . $secureShellCustomConfig;
	...
	protected function installUnattendedUpgrades() {
		...
		// Enable UnattendedUpgrades to run every day
		...
		if(ISPConfig::shallInstall('mail') && !ISPConfig::wantsAmavis()) {
			$unattendedupgrades .= '        "origin=Rspamd";' . "\n";
		}

		// sury php repository
		if(ISPConfig::wantsPHP() !== 'system') {
			$unattendedupgrades .= '        "origin=deb.sury.org";' . "\n";
		}

		// GoAccess repository
		if(ISPConfig::shallInstall('web')) {
			$unattendedupgrades .= '        "origin=GoAccess Repository";' . "\n";
		}

		// Enable repositories & {distro_codename}-updates
		$unattendedupgrades  = 'Unattended-Upgrade::Origins-Pattern {' . "\n" . $unattendedupgrades;
		$unattendedupgrades .= '        "origin=Debian,codename=${distro_codename}-updates";' . "\n";
		$unattendedupgrades .= '}' . "\n";

		// Enable mail to root
		$unattendedupgrades .= 'Unattended-Upgrade::Mail "root";';

		file_put_contents('/etc/apt/apt.conf.d/51unattended-upgrades', $unattendedupgrades);
	}
...
mailman-unsubscribe:  "|/var/lib/mailman/mail/mailman unsubscribe mailman"';
	...
	protected function installRoundcube($mysql_root_pw) {
		...
		$this->replaceContents('/etc/roundcube/config.inc.php', $replacements);
		...
		if(ISPConfig::$WEBSERVER === ISPC_WEBSERVER_APACHE) {
			$replacements = array(
				...
				'/(<Directory \/var\/lib\/roundcube\/public\_html\/>\n)(.*\n)(.*)/' => [
					'replace' => '$1$2  DirectoryIndex index.php' . "\n" . '$3',
					'ifnot' => 'DirectoryIndex'
				],
			);
			$this->replaceContents('/etc/roundcube/apache.conf', $replacements);
		} elseif(ISPConfig::$WEBSERVER === ISPC_WEBSERVER_NGINX) {
	...
	public function runPerfectSetup() {
		$log_filename = 'setup-' . date('YmdHis') . '.log';
		...
		// Dash to bash change only possible until Debian 11
		...
		if(in_array('rsyslog', $packages)) {
			if(is_dir('/var/log/journal')) {
				$this->exec('rm -rf /var/log/journal');
				$this->restartService('systemd-journald');
			}
		}
		...
		ISPConfigLog::info('Configuring rkhunter.', true);
		$contents = 'MAIL-ON-WARNING=root
ALLOW_SSH_ROOT_USER=unset
SCRIPTWHITELIST=/usr/bin/lwp-request';
		file_put_contents('/etc/rkhunter.conf.local', $contents);
		...
		if(ISPConfig::shallInstall('mail') && !ISPConfig::wantsAmavis()) {
			...
			$replacements = array(
				'/^\s*nrows\s*=.*$/m'           => 'nrows = 2500;',
				'/^\s*compress\s*=.*$/m'        => 'compress = true;',
				'/^\s*subject_privacy\s*=.*$/m' => 'subject_privacy = true;',
			);
			$this->replaceContents('/etc/rspamd/local.d/history_redis.conf', $replacements, true);
		}
		...
		*/
		...
		// Fail2Ban is disabled by default on Ubuntu 22.04 and newer. Enabling it on any OS does not hurt.
		$cmd = 'systemctl enable fail2ban >/dev/null 2>&1';
		$this->exec($cmd); // ignore if this fails
